...
new_tet_mesh_subfolders: list[Path] = list()
removed_labeling_subfolders: list[Path] = list()

root_folder = root() # get the root data folder
for level_minus_1_folder in [x for x in root_folder.path.iterdir() if x.is_dir()]: # for each folder inside
    ...
    if not (level_minus_1_folder / 'Gmsh_0.1').exists():
        ...
        new_tet_mesh_subfolders.append(level_minus_1_folder / 'Gmsh_0.1')
        continue
    ...
    if len(labeling_subfolders_generated_by_automatic_polycube) == 0:
        ...
        # there is already a labeling generated by automatic polycube
        ...
        try:
            ...
            if print_content_and_propose_to_remove(labeling_subfolders_generated_by_automatic_polycube[0]):
                removed_labeling_subfolders.append(labeling_subfolders_generated_by_automatic_polycube[0])
            continue
        if labeling_folder.has_valid_labeling() and labeling_folder.nb_turning_points() == 0:
            continue
        else:
            # invalid labeling and/or non-monotone boundaries
            print(f'Labeling {labeling_subfolders_generated_by_automatic_polycube[0]} is invalid or has turning-points. Content of the folder:')
            if print_content_and_propose_to_remove(labeling_subfolders_generated_by_automatic_polycube[0]):
                removed_labeling_subfolders.append(labeling_subfolders_generated_by_automatic_polycube[0])
        
if len(new_tet_mesh_subfolders) != 0:
    print('The following tet mesh(es) have been generated, re-run the script to generate a labeling.')
    for subfolder in new_tet_mesh_subfolders:
        print(str(subfolder.relative_to(root_folder.path)))
    print(new_tet_mesh_subfolders)
if len(removed_labeling_subfolders) != 0:
    print('The following labeling(s) have been removed, re-run the script to generate a new labeling.')
    for subfolder in removed_labeling_subfolders:
        print(str(subfolder.relative_to(root_folder.path)))