...
class root(AbstractDataFolder):
    ...
    def batch_processing(self):
        ...
        for step_subfolder in self.get_subfolders_of_type('step'): # for each folder inside
            ...
            # keep labeling generated by 'automatic_polycube', no recomputation
            # get labeling subfolders generated by 'evocube'
            labeling_subfolders_generated_by_evocube: list[Path] = tet_folder.get_subfolders_generated_by('evocube')
            assert(len(labeling_subfolders_generated_by_evocube) <= 1) # expecting 0 or 1 labeling generated by this algo
            # try to compute one if none
            if(len(labeling_subfolders_generated_by_evocube) == 0):
                tet_folder.evocube()
            labeling_subfolders_generated_by_evocube: list[Path] = tet_folder.get_subfolders_generated_by('evocube')
            assert(len(labeling_subfolders_generated_by_evocube) <= 1)
            if(len(labeling_subfolders_generated_by_evocube) == 1):
