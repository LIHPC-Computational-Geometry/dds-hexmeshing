...
def main(input_folder: Path, arguments: list):
    ...
    def process_Our_output(tet_mesh_object: DataFolder, row_template: dict) -> tuple[Optional[float],Optional[DataFolder]]:
        ...
        graphcut_row['percentage_removed']       = None if total_feature_edges == 0 else labeling_stats['feature-edges']['removed']/total_feature_edges*100
        graphcut_row['percentage_lost']          = None if total_feature_edges == 0 else labeling_stats['feature-edges']['lost']/total_feature_edges*100
        graphcut_row['percentage_preserved']     = None if total_feature_edges == 0 else labeling_stats['feature-edges']['preserved']/total_feature_edges*100
    ...
    def process_Evocube_output(tet_mesh_object: DataFolder, row_template: dict, Ours_duration: Optional[float], Ours_labeling: Optional[DataFolder]):
        ...
        if ( (len(labeling_subfolders_generated_by_evocube) == 0) or \
            ...
            total_feature_edges = labeling_stats['feature-edges']['removed'] + labeling_stats['feature-edges']['lost'] + labeling_stats['feature-edges']['preserved']
            assert(total_feature_edges == surface_mesh_stats['edges']['nb'])
            evocube_row['percentage_removed']       = None if total_feature_edges == 0 else labeling_stats['feature-edges']['removed']/total_feature_edges*100
            evocube_row['percentage_lost']          = None if total_feature_edges == 0 else labeling_stats['feature-edges']['lost']/total_feature_edges*100
            evocube_row['percentage_preserved']     = None if total_feature_edges == 0 else labeling_stats['feature-edges']['preserved']/total_feature_edges*100
    ...
    print(f'Parsing results on MAMBO...')
    ...
    print(f'Parsing results on OctreeMeshing/cad...')
    for depth_1_folder in sorted(get_subfolders_of_type(input_folder / 'OctreeMeshing' / 'cad','tet-mesh')):
        depth_1_object: Optional[DataFolder] = None
        try:
            # instantiate this depth-1 folder
            depth_1_object = DataFolder(depth_1_folder)
            if(depth_1_object.type != 'tet-mesh'):
                log.warning(f"Found a depth-1 folder that is not of type 'tet-mesh' but '{depth_1_object.type}': {depth_1_folder}")
                continue
            assert((depth_1_folder / SURFACE_MESH_OBJ_filename).exists())
        except DataFolderInstantiationError:
            log.warning(f"Found a depth-1 folder that cannot be instantiated: {depth_1_folder}")
            continue

        CAD_name = depth_1_folder.name
        surface_mesh_stats = depth_1_object.get_surface_mesh_stats_dict() # type: ignore | see ../data_folder_types/tet-mesh.accessors.py

        fluxes[VOID,OCTREE_MESHING_CAD] += 1
        fluxes[OCTREE_MESHING_CAD,CAD] += 1
        fluxes[CAD,TET_MESHING_SUCCESS] += 1 # we reused Evocube tet-meshes, all succeeded

        # prepare the AG Grid rows content
        row_template = dict() # all columns to None expect the 'CAD_name'
        row_template['CAD_name']                 = CAD_name
        row_template['method']                   = None
        row_template['nb_vertices']              = surface_mesh_stats['vertices']['nb']
        row_template['nb_facets']                = surface_mesh_stats['facets']['nb']
        row_template['area_sd']                  = surface_mesh_stats['facets']['area']['sd']
        row_template['nb_charts']                = None
        row_template['nb_boundaries']            = None
        row_template['nb_corners']               = None
        row_template['nb_invalid_charts']        = None
        row_template['nb_invalid_boundaries']    = None
        row_template['nb_invalid_corners']       = None
        row_template['min_fidelity']             = None
        ...
        row_template['percentage_removed']       = None
        row_template['percentage_lost']          = None
        row_template['percentage_preserved']     = None
        row_template['minSJ']                    = None
        row_template['avgSJ']                    = None
        row_template['glb_hexmesh']              = None
    ...
    node_quantity = dict()
    node_quantity[MAMBO_BASIC] = intermediate_node_quantity(fluxes,MAMBO_BASIC)
    assert(node_quantity[MAMBO_BASIC] == 74)
    node_quantity[MAMBO_SIMPLE] = intermediate_node_quantity(fluxes,MAMBO_SIMPLE)
    assert(node_quantity[MAMBO_SIMPLE] == 30)
    node_quantity[MAMBO_MEDIUM] = intermediate_node_quantity(fluxes,MAMBO_MEDIUM)
    assert(node_quantity[MAMBO_MEDIUM] == 9)
    node_quantity[OCTREE_MESHING_CAD] = intermediate_node_quantity(fluxes,OCTREE_MESHING_CAD)
    assert(node_quantity[OCTREE_MESHING_CAD] == 109)
    ...
    if node_quantity[MAMBO_BASIC] != 0:
        node_name_to_index[MAMBO_BASIC] = nb_nodes
        nb_nodes += 1
        Sankey_diagram_data["nodes"].append({
            "node": node_name_to_index[MAMBO_BASIC],
            "name": f"MAMBO/Basic ({node_quantity[MAMBO_BASIC]})"
        })
    if node_quantity[MAMBO_SIMPLE] != 0:
        node_name_to_index[MAMBO_SIMPLE] = nb_nodes
        nb_nodes += 1
        Sankey_diagram_data["nodes"].append({
            "node": node_name_to_index[MAMBO_SIMPLE],
            "name": f"MAMBO/Simple ({node_quantity[MAMBO_SIMPLE]})"
        })
    if node_quantity[MAMBO_MEDIUM] != 0:
        node_name_to_index[MAMBO_MEDIUM] = nb_nodes
        nb_nodes += 1
        Sankey_diagram_data["nodes"].append({
            "node": node_name_to_index[MAMBO_MEDIUM],
            "name": f"MAMBO/Medium ({node_quantity[MAMBO_MEDIUM]})"
        })
    if node_quantity[OCTREE_MESHING_CAD] != 0:
        node_name_to_index[OCTREE_MESHING_CAD] = nb_nodes
        nb_nodes += 1
        Sankey_diagram_data["nodes"].append({
            "node": node_name_to_index[OCTREE_MESHING_CAD],
            "name": f"OctreeMeshing/cad ({node_quantity[OCTREE_MESHING_CAD]})"
        })
    if node_quantity[CAD] != 0:
        node_name_to_index[CAD] = nb_nodes
        nb_nodes += 1
        Sankey_diagram_data["nodes"].append({
    ...
    if node_quantity[LABELING_SUCCESS] != 0:
        ...
        Sankey_diagram_data["nodes"].append({
            ...
            "name": f"valid and all-monotone labeling: {node_quantity[LABELING_SUCCESS]/node_quantity[TET_MESHING_SUCCESS]*100:.2f} %"
        })
    ...
    if node_quantity[HEX_MESHING_NEGATIVE_MIN_SJ] != 0:
        ...
        Sankey_diagram_data["nodes"].append({
            ...
            "name": f"hex-mesh with minSJ<0: {node_quantity[HEX_MESHING_NEGATIVE_MIN_SJ]/max_nb_hexmeshes*100:.2f} %"
        })
    if node_quantity[HEX_MESHING_POSITIVE_MIN_SJ] != 0:
        ...
        Sankey_diagram_data["nodes"].append({
            ...
            "name": f"hex-mesh with minSJ≥0: {node_quantity[HEX_MESHING_POSITIVE_MIN_SJ]/max_nb_hexmeshes*100:.2f} %"
        })
