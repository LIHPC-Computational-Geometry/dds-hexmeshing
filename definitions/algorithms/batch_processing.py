...
def main(input_folder: Path, arguments: list, silent_output: bool):
    ...
    for step_subfolder in get_subfolders_of_type(input_folder,'step'):
        ...
        for labeling_subfolder in labeling_subfolders_to_look_into:
            ...
            if not (hex_mesh_object.path / 'global_padding').exists():
                with console.status(f"Running '[b]global_padding[/]' on [cyan]{collapseuser(hex_mesh_object.path)}[/]") as status:
                    hex_mesh_object.run('global_padding', silent_output=True)
                # here we assert global_padding succeeded
                console.print(f"\[[green]✓[/]] '[b]global_padding[/]' on [cyan]{collapseuser(hex_mesh_object.path)}[/]")
            else:
                console.print(f"\[-] '[b]global_padding[/]' already run on [cyan]{collapseuser(hex_mesh_object.path)}[/]")