...
def main(input_folder: Path, arguments: list):
    ...
    for level_minus_1_folder in get_subfolders_of_type(input_folder / 'MAMBO', 'step'):
        ...
        if not (level_minus_1_folder / 'Gmsh_0.15' / 'PolyCut_3').exists() or not (level_minus_1_folder / 'Gmsh_0.15' / 'PolyCut_3' / surface_labeling_filename).exists():
            # there is a tet mesh but no labeling was written
            fluxes[MAMBO_subset_id,POLYCUT,COARSER_TET_MESHING_SUCCESS,LABELING_FAILURE] += 1
        else:
            # instantiate the labeling folder
            labeling_folder: DataFolder = DataFolder(level_minus_1_folder / 'Gmsh_0.15' / 'PolyCut_3')
            assert(labeling_folder.type == 'labeling')
            ...
            # update avg fidelity sum
            sum_avg_fidelities[MAMBO_subset_id,POLYCUT] += labeling_stats['fidelity']['avg']

            # there is no feature edges in the PolyCut input & output meshes, don't count feature edges

            # update duration sum
            labeling_duration[MAMBO_subset_id,POLYCUT] += polycut_durations['polycut'] # should we take into account the duration of cusy2.exe, which is the executable writing the labeling?
            ...
            if (labeling_folder.path / 'optimizer_100' / 'untangler' / hex_mesh_filename).exists():
                ...
                try:
                    ...
                    avg_sj_sum[MAMBO_subset_id,POLYCUT] += -1.0 # assume worse value
                    min_sj_sum[MAMBO_subset_id,POLYCUT] += -1.0 # assume worse value
                    pass # failure to generate a hex-mesh stats JSON because .mesh with no cells and no vertices !
                if len(hex_mesh_stats) > 0:
                    if 'quality' in hex_mesh_stats['cells']: 
                        avg_sj_sum[MAMBO_subset_id,POLYCUT] += hex_mesh_stats['cells']['quality']['hex_SJ']['avg']
                        hexmesh_minSJ = hex_mesh_stats['cells']['quality']['hex_SJ']['min']
                        min_sj_sum[MAMBO_subset_id,POLYCUT] += hexmesh_minSJ
                    else:
            ...
            # update the counters
            if not labeling_folder.has_valid_labeling(): # type: ignore | see ../data_folder_types/labeling.accessors.py
                fluxes[MAMBO_subset_id,POLYCUT,COARSER_TET_MESHING_SUCCESS,LABELING_INVALID] += 1
            elif labeling_folder.nb_turning_points() != 0: # type: ignore | see ../data_folder_types/labeling.accessors.py
                fluxes[MAMBO_subset_id,POLYCUT,COARSER_TET_MESHING_SUCCESS,LABELING_NON_MONOTONE] += 1
                if hexmesh_minSJ is not None:
                    # an hex-mesh was successfully generated
                    if hexmesh_minSJ < 0.0:
                        fluxes[MAMBO_subset_id,POLYCUT,LABELING_NON_MONOTONE,HEX_MESHING_NEGATIVE_MIN_SJ] += 1
                    else:
                        fluxes[MAMBO_subset_id,POLYCUT,LABELING_NON_MONOTONE,HEX_MESHING_POSITIVE_MIN_SJ] += 1
                else:
                    # no hex-mesh
                    fluxes[MAMBO_subset_id,POLYCUT,LABELING_NON_MONOTONE,HEX_MESHING_FAILURE] += 1
            else:
                fluxes[MAMBO_subset_id,POLYCUT,COARSER_TET_MESHING_SUCCESS,LABELING_SUCCESS] += 1
                if hexmesh_minSJ is not None:
                    # an hex-mesh was successfully generated
                    if hexmesh_minSJ < 0.0:
                        fluxes[MAMBO_subset_id,POLYCUT,LABELING_SUCCESS,HEX_MESHING_NEGATIVE_MIN_SJ] += 1
                    else:
                        fluxes[MAMBO_subset_id,POLYCUT,LABELING_SUCCESS,HEX_MESHING_POSITIVE_MIN_SJ] += 1
                else:
                    # no hex-mesh
                    fluxes[MAMBO_subset_id,POLYCUT,LABELING_SUCCESS,HEX_MESHING_FAILURE] += 1
    ...
    for MAMBO_prefix in ['B','S','M']:
        ...
        if SHOW_EVOCUBE_STATS:
            ...
            duration_factor_relative_to_Ours_2024_09 = labeling_duration[MAMBO_subset_id,EVOCUBE] / (labeling_duration[MAMBO_subset_id,GRAPHCUT]+labeling_duration[MAMBO_subset_id,OURS_2024_09])
        ...
        if SHOW_POLYCUT_STATS:
            assert(fluxes[MAMBO_subset_id,N_A,CAD,COARSER_TET_MESHING_FAILURE] == 0) # expect all tet-mesh generations succeeded. easier for the stats
            percentage_labeling_success = fluxes[MAMBO_subset_id,POLYCUT,COARSER_TET_MESHING_SUCCESS,LABELING_SUCCESS] / nb_CAD_models * 100
            percentage_labeling_non_monotone = fluxes[MAMBO_subset_id,POLYCUT,COARSER_TET_MESHING_SUCCESS,LABELING_NON_MONOTONE] / nb_CAD_models * 100
            percentage_labeling_invalid = fluxes[MAMBO_subset_id,POLYCUT,COARSER_TET_MESHING_SUCCESS,LABELING_INVALID] / nb_CAD_models * 100
            percentage_labeling_failure = fluxes[MAMBO_subset_id,POLYCUT,COARSER_TET_MESHING_SUCCESS,LABELING_FAILURE] / nb_CAD_models * 100
            nb_labeling_generated = \
                fluxes[MAMBO_subset_id,POLYCUT,COARSER_TET_MESHING_SUCCESS,LABELING_SUCCESS] + \
                fluxes[MAMBO_subset_id,POLYCUT,COARSER_TET_MESHING_SUCCESS,LABELING_NON_MONOTONE] + \
                fluxes[MAMBO_subset_id,POLYCUT,COARSER_TET_MESHING_SUCCESS,LABELING_INVALID]
            overall_average_fidelity = sum_avg_fidelities[MAMBO_subset_id,POLYCUT] / nb_labeling_generated
            duration_factor_relative_to_Ours_2024_09 = labeling_duration[MAMBO_subset_id,POLYCUT] / (labeling_duration[MAMBO_subset_id,GRAPHCUT]+labeling_duration[MAMBO_subset_id,OURS_2024_09])
            nb_tried_hex_meshing = \
                fluxes[MAMBO_subset_id,POLYCUT,COARSER_TET_MESHING_SUCCESS,LABELING_SUCCESS] + \
                fluxes[MAMBO_subset_id,POLYCUT,COARSER_TET_MESHING_SUCCESS,LABELING_NON_MONOTONE]
            nb_hex_meshes_with_positive_min_sj = \
                fluxes[MAMBO_subset_id,POLYCUT,LABELING_SUCCESS,HEX_MESHING_POSITIVE_MIN_SJ] + \
                fluxes[MAMBO_subset_id,POLYCUT,LABELING_NON_MONOTONE,HEX_MESHING_POSITIVE_MIN_SJ]
            percentage_hex_mesh_positive_min_SJ = nb_hex_meshes_with_positive_min_sj / nb_tried_hex_meshing * 100
            average_min_SJ = min_sj_sum[MAMBO_subset_id,POLYCUT] / nb_tried_hex_meshing
            average_avj_SJ = avg_sj_sum[MAMBO_subset_id,POLYCUT] / nb_tried_hex_meshing
            table.add_row(
                f'MAMBO/{MAMBO_subset_str} ({nb_CAD_models})',
                'PolyCut',
                f"{percentage_labeling_success:.1f} %\n{percentage_labeling_non_monotone:.1f} %\n{percentage_labeling_invalid:.1f} %\n{percentage_labeling_failure:.1f} %",
                f"{overall_average_fidelity:.3f}",
                "-",
                f"{labeling_duration[MAMBO_subset_id,POLYCUT]:.3f}† s\n(x{duration_factor_relative_to_Ours_2024_09:.1f} Ours_2024-09)",
                f"{percentage_hex_mesh_positive_min_SJ:.1f} %",
                f"{average_min_SJ:.3f}\n{average_avj_SJ:.3f}"
            )
            table.add_section()
