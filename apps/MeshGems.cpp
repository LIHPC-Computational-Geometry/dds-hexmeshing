...
int main(int argc, char *argv[]) {
    ...
    std::string basename = (input_as_path.extension() == ".txt") ? 
                            input_as_path.stem().string() + "_MeshGems_" + global_beginning.filename_string() : //if the input is a collection
                            "MeshGems"; //if the input is a single folder
    OutputCollections output_collections(basename,path_list,result.is_specified("no-output-collections"));
    output_collections.set_header("MeshGems",global_beginning.pretty_string(),result["comments"]);
    ...
    for(auto& input_folder : input_folders) {
        ...
        if(returncode!=0) {
            ...
            regenerate_Graphite_visu(path_list[WORKING_DATA_FOLDER],input_folder / output_folder_name,current_input_beginning,"the MeshGems wrapper");
        }
    }
