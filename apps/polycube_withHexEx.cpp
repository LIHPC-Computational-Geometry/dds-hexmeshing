...
int main(int argc, char *argv[]) {
    ...
    for(auto& input_folder : input_folders) {
        ...
        cmd = (path_list[EVOCUBE_TWEAKS] / "polycube_withHexEx").string() + " " +
              (parent_of(input_folder) / TETRA_MESH_FILE).string() + " " +
        ...
        //TODO write .geogram containing hex mesh + per hex Scaled Jacobian

        //then create a Lua script for Graphite
        GraphiteScript graphite_script(input_folder / output_folder_name / HEX_MESH_LUA_SCRIPT,path_list);
        ...
        graphite_script.load_object(HEX_MESH_FILE);
    ...
    if(input_folders.size()==1 && returncode==0) { //TODO if returncode!=0, open the logs
        // TODO check if HEX_MESH_LUA_SCRIPT and GRAPHITE_BASH_SCRIPT were successfully created
